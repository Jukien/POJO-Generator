...
public class Util {
    ...
    public static LinkedHashSet<Field> getFields(DbTable dbTable, JPAMappingSettings jpaMappingSettings) {
        ...
        for (DasColumn column : DasUtil.getColumns(dbTable)) {
            ...
            ConfigurableJPAMapping configurableJPAMapping = jpaMappings.stream()
                    .filter(c -> dbTable.getDataSource().getDatabaseVersion().name.equals(c.getFamily().getName()) &&
                            column.getDasType().toDataType().typeName.equals(c.getSqlDataType()))
                    .findAny()
                    .orElse(null);
            ...
            if (null != configurableJPAMapping) {
                ...
                if (null != configurableJPAMapping.isLengthAttributeEnabled() && configurableJPAMapping.isLengthAttributeEnabled()) {
                    field.setLength(column.getDasType().toDataType().size);
                }
            } else {
    ...
    public static void addConstructor(String className, LinkedHashSet<Field> fields, StringBuilder javaTextFile) {
    ...
    public static void addGetterSetter(LinkedHashSet<Field> fields, StringBuilder javaTextFile) {
        int index = 0;
        for (Field field : fields) {
    ...
    public static void createFile(Project project, StringBuilder javaTextFile, String fileName, POJOGeneratorSettings pojoGeneratorSettings) {
        ...
        if (null == psiDirectory.findFile(file.getName())) {
            ...
            Notification notification = new Notification("POJO Generator", "POJO Generator", String.format("The file [%s] already exists", fileName), NotificationType.WARNING);
